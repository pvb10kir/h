...
local function pre_process(msg)
	...
	if expiretime then    
		...
		rem_mutes(msg.to.id)
		superrem2(msg)
		return send_large_msg(get_receiver(msg), ' تاریخ اتقضای گروه به پایان رسید و ربات دیگر در گروهتون کار نخواهد کرد.\n از پشتیبانی در خواست تمدید کنید.\n@Sphero_bot (بخش پیام رسان)')
		else
			return
		end
	end
	if tonumber(timetoexpire) == 0 then
		...
		send_large_msg(get_receiver(msg), '0 روز تا پایان تاریخ انقضای گروه باقی مانده است\nنرخ تمدید : https://telegram.me/Sphero_Ch/339\nتمدید در @Sphero_Bot (بخش امور ربات اسفرو/تمدید گروه)')
